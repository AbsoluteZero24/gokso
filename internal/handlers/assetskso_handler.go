...
func (server *Server) SubmitMaintenance(w http.ResponseWriter, r *http.Request) {
	...
	// 2. Link all unbatched reports for this period/filter/category to this document
	// We need to join with asset_kso to filter by category
	reportIDs := []string{}
	subQuery := server.DB.Table("maintenance_reports").
		Select("maintenance_reports.id").
		Joins("JOIN asset_kso ON asset_kso.id = maintenance_reports.asset_id").
		Where("maintenance_reports.period = ? AND maintenance_reports.document_id IS NULL", period).
		Where("asset_kso.category = ?", category)

	if branch != "" {
		subQuery = subQuery.Where("maintenance_reports.user_branch = ?", branch)
	}
	if dept != "" {
		subQuery = subQuery.Where("maintenance_reports.user_department = ?", dept)
	}
	if subDept != "" {
		subQuery = subQuery.Where("maintenance_reports.user_sub_department = ?", subDept)
	}

	subQuery.Scan(&reportIDs)

	if len(reportIDs) > 0 {
		err = server.DB.Model(&models.MaintenanceReport{}).
...
func (server *Server) ApproveMaintenance(w http.ResponseWriter, r *http.Request) {
	...
	err = server.DB.Where("category = ? AND branch = ? AND department = ? AND sub_department = ? AND period = ? AND status = ?",
		category, branch, dept, subDept, period, "Submitted").First(&doc).Error
